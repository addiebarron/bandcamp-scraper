...
const validateSearchResult = require('../schemas/search-result');
...
exports.parseSearchResults = function (html) {
  ...
  return data.results.reduce(function (results, result) {
    ...
    switch (result.type) {
      ...
      case 'track':
        ...
        if (result.subhead) {
          ...
          if (info.length > 0) {
            object.album = removeMultipleSpace(info[0])
              .replace('location', '')
              .replace(/^from /, '');
            info.shift();
    ...
    if (validateSearchResult(object)) {
      results.push(object);
    } else {
      console.error('Validation error on search result: ', object);
    }
    return results;
  }, []);
};
